...
public:
    ...
    void updateVelocities(double dt, bool add_swimming = true)
    {
        for (auto &bead : beads)
        {
            ...
            // Update velocity without damping as requested
            bead.vx += ax * dt;
            bead.vy += ay * dt;
        }
    }
    ...
    void updatePositions(double dt)
    {
        ...
        // Unit vector along head-tail direction
        ...
        for (auto &bead : beads)
        {
            // Apply pure constant swimming translation without accumulating in vx
            bead.x += (bead.vx + vSwimX) * dt;
            bead.y += (bead.vy + vSwimY) * dt;
        }
    }

    // Override step to use the new updateVelocities and updatePositions
    void step(double dt, double t)
    {
        computeInternalForces(t);
        updateVelocities(dt);
